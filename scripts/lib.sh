...
load_config() {
  ...
  : "${CACHYOS_ARCH_LEVEL:=znver4}" "${DOTS_MONITOR:=HDMI-A-1}" "${DOTS_SDDM_THEME:=sddm-astronaut-theme}" "${NXAPI_VERSION:=1.6.1-next.254}" "${DOTS_BTRFS_EXTRA_DEVICE:=}" "${DOTS_BTRFS_WIPE:=0}" "${DOTS_VM:=0}" "${DOTS_BITDEPTH:=10}" "${DOTS_LIMINE_DEFAULT_ENTRY:=2}" "${DOTS_LIMINE_TIMEOUT:=3}"
  export DOTS_USER DOTS_HOSTNAME DOTS_TIMEZONE DOTS_LOCALE DOTS_KEYMAP CACHYOS_ARCH_LEVEL DOTS_MONITOR DOTS_SDDM_THEME NXAPI_VERSION DOTS_BTRFS_EXTRA_DEVICE DOTS_BTRFS_WIPE DOTS_VM DOTS_BITDEPTH DOTS_LIMINE_DEFAULT_ENTRY DOTS_LIMINE_TIMEOUT
}

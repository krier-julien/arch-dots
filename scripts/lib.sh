...
load_config() {
  ...
  : "${CACHYOS_ARCH_LEVEL:=znver4}" "${DOTS_MONITOR:=HDMI-A-1}" "${DOTS_SDDM_THEME:=sddm-astronaut-theme}" "${NXAPI_VERSION:=1.6.1-next.254}" "${DOTS_BTRFS_EXTRA_DEVICE:=}" "${DOTS_BTRFS_WIPE:=0}" "${DOTS_VM:=0}" "${DOTS_BITDEPTH:=10}" "${DOTS_LIMINE_DEFAULT_ENTRY:=2}" "${DOTS_LIMINE_TIMEOUT:=3}" "${DOTS_NVIDIA_EARLY_KMS:=0}"
